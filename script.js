...
const STAT_DEFS = [
    ...
    { id: 'magic',   name: '魔力',   key: 'magic',   max: 100 },
    { id: 'dex',     name: '器用さ', key: 'dex',     max: 95 },
    { id: 'defense', name: '耐久力', key: 'defense', max: 95 },
    { id: 'mental',  name: '精神力', key: 'mental',  max: 90 }
];
...
/**
 * 改良版：長押し処理
 * 最初の1クリック目は即座に反応し、500ms待ってから連続加算を開始
 */
...
function loadData() {
    ...
    if (params.has('hp') || params.has('p') || params.has('s')) {
        ...
        if (saved) STAT_DEFS.forEach(s => { 
            const sIn = document.getElementById(`${s.id}-start`);
            const tIn = document.getElementById(`${s.id}-target-input`);
            const kIn = document.getElementById(`${s.id}-kotsu`);
            if(sIn) sIn.value = saved.starts[s.id] || 1; 
            if(tIn) { tIn.value = saved.targets[s.id] || 1; updateRankDisplay(s.id, tIn.value); }
            if(kIn) kIn.checked = saved.kotsu[s.id] || false; 
        });
    }
    calculateTotal();
}
